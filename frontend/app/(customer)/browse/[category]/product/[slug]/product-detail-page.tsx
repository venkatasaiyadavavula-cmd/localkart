...
import {
  ...
  X, ShoppingBag, CheckCircle, Clock,
  ChevronRight,
} from 'lucide-react';
...
export default function ProductDetailPage() {
  ...
  const { data: wishlistData } = useQuery({
    queryKey: ['wishlist'],
    queryFn: async () => {
      const { data } = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/users/wishlist`, { headers: authHeader() });
      return data as string[];
    },
    enabled: typeof window !== 'undefined' && !!localStorage.getItem('accessToken'),
  });

  const isWishlisted = wishlistData?.includes(product?.id) ?? false;

  const wishlistMutation = useMutation({
    mutationFn: async () => {
      const { data } = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/users/wishlist/toggle`,
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto lg:px-6 lg:pb-8">
        <div className="lg:grid lg:grid-cols-2 lg:gap-10">
          ...
          <div className="bg-white lg:rounded-2xl overflow-hidden">
            <div className="relative bg-gray-50">
              ...
              <div className="absolute top-4 left-4 flex flex-col gap-2">
                {discount > 0 && (
                  <span className="bg-red-500 text-white text-xs font-bold px-2.5 py-1 rounded-lg">{discount}% OFF</span>
                )}
                {product.stock <= 5 && product.stock > 0 && (
                  <span className="bg-orange-500 text-white text-xs font-bold px-2.5 py-1 rounded-lg">Only {product.stock} left!</span>
                )}
              </div>
              {product.videos?.length > 0 && (
                <button onClick={() => setShowVideo(true)}
                  className="absolute bottom-4 left-4 flex items-center gap-2 bg-black/70 text-white text-xs font-semibold px-3 py-2 rounded-full">
              ...
              <div className="hidden lg:flex absolute top-4 right-4 flex-col gap-2">
                ...
                <button onClick={handleShare} className="bg-white p-2.5 rounded-full shadow-md">
                  <Share2 className="h-5 w-5 text-gray-400" />
                </button>
              </div>
            </div>
            {product.images?.length > 1 && (
              <div className="flex gap-2 px-4 py-3 overflow-x-auto border-t bg-white no-scrollbar">
                {product.images.map((img: string, i: number) => (
                  <button key={i} onClick={() => setSelectedImage(i)}
                    className={cn('relative flex-shrink-0 w-14 h-14 rounded-lg overflow-hidden border-2 transition-all',
          ...
          <div className="bg-white lg:rounded-2xl p-4 lg:p-6 mt-2 lg:mt-0">
            <Link href={`/shop/${product.shop?.slug}`}
              className="inline-flex items-center gap-2 text-xs text-primary font-semibold bg-primary/10 px-3 py-1.5 rounded-full mb-3">
            ...
            <div className="flex items-center gap-3 mb-4">
              <div className="flex items-center gap-1.5 bg-green-50 px-2.5 py-1 rounded-lg">
                ...
                <span className="text-xs font-bold text-green-700">{product.rating ? product.rating.toFixed(1) : '—'}</span>
              </div>
              ...
              {product.orderCount > 0 && (
                ...
                <span className="text-xs text-gray-500">{product.orderCount}+ sold</span></>
              )}
            </div>
            ...
            <div className="flex items-center gap-2 mb-4">
              {product.stock > 0
                ? <span className="flex items-center gap-1.5 text-xs font-semibold text-green-600"><CheckCircle className="h-3.5 w-3.5" /> In Stock ({product.stock} available)</span>
              ...
              <div className="flex items-center gap-3">
                <div className="flex items-center border border-gray-200 rounded-xl overflow-hidden">
                  <button onClick={() => setQuantity(Math.max(1, quantity - 1))} disabled={quantity <= 1}
                    className="px-3 py-2 hover:bg-gray-50 disabled:opacity-40 transition-colors"><Minus className="h-4 w-4" /></button>
                  <span className="w-10 text-center text-base font-bold">{quantity}</span>
                  <button onClick={() => setQuantity(Math.min(product.stock || 99, quantity + 1))} disabled={quantity >= (product.stock || 99)}
                    className="px-3 py-2 hover:bg-gray-50 disabled:opacity-40 transition-colors"><Plus className="h-4 w-4" /></button>
                </div>
            ...
            <div className="flex gap-3 mb-5">
              <Button size="lg" variant="outline" className="flex-1 border-primary text-primary font-bold"
                onClick={handleAddToCart} disabled={cartLoading || product.stock === 0}>
                {addedToCart
            ...
            <div className="space-y-2.5 bg-blue-50/50 rounded-xl p-4 border border-blue-100">
              {[
                { icon: Truck,     title: 'Same Day Delivery',   sub: 'Order before 4 PM',              color: 'text-blue-500' },
                { icon: Shield,    title: 'Secure Payment',       sub: 'COD & online payments accepted', color: 'text-green-500' },
                { icon: RotateCcw, title: '24h Easy Returns',     sub: 'Return within 24 hours',         color: 'text-orange-500' },
                { icon: Clock,     title: 'Local Shop',           sub: 'Fresh from your neighborhood',   color: 'text-purple-500' },
        ...
        <div className="bg-white mt-2 lg:mt-6 lg:rounded-2xl">
          <Tabs defaultValue="description" className="w-full">
            <TabsList className="w-full justify-start border-b bg-transparent p-0 h-auto rounded-none">
              {['description', 'specifications', 'reviews'].map(tab => (
                <TabsTrigger key={tab} value={tab}
                  className="capitalize rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:text-primary data-[state=active]:bg-transparent px-5 py-3 text-sm font-semibold">
            ...
            {/* ── REVIEWS TAB — real component ── */}
            <TabsContent value="reviews" className="px-4 py-5 space-y-5">
              <ReviewForm
                productId={product.id}
      ...
      <div className="lg:hidden fixed bottom-16 left-0 right-0 z-30 bg-white border-t px-4 py-3 flex gap-3 shadow-[0_-4px_20px_rgba(0,0,0,0.08)]">
        <Button variant="outline" className="flex-1 border-primary text-primary font-bold"
          onClick={handleAddToCart} disabled={cartLoading || product.stock === 0}>
      ...
      <Dialog open={showVideo} onOpenChange={setShowVideo}>
        <DialogContent className="max-w-3xl p-0 overflow-hidden rounded-2xl">
          <button onClick={() => setShowVideo(false)} className="absolute right-3 top-3 z-10 rounded-full bg-black/60 p-2 text-white">
            <X className="h-4 w-4" />
          </button>
