...
function VerifyOtpForm() {
  ...
  const handleVerify = async () => {
    ...
    setIsVerifying(true);
    try {
      await verifyOtp(phone, otp, mode, orderId);
      if (mode === 'register') { toast.success('Phone verified!'); router.push('/login'); }
  ...
  const handleResendOtp = async () => {
    ...
    setIsResending(true);
    try {
      await sendOtp(phone, mode, orderId);
  ...
  const getTitleAndDescription = () => {
    switch (mode) {
      case 'register': return { title: 'Verify Your Phone', description: 'Enter the 6-digit code sent to your phone to complete registration.' };
      case 'login': return { title: 'Login with OTP', description: 'Enter the 6-digit code sent to your phone to sign in.' };
      case 'order': return { title: 'Confirm Your Order', description: 'Enter the 6-digit OTP sent to your phone to confirm this order.' };
      default: return { title: 'Verify OTP', description: 'Enter the 6-digit verification code.' };
    }
  };
    ...
    <div style={{ position: 'relative', zIndex: 10 }}>
      ...
      <div className="space-y-8">
        <div className="flex justify-center">
          <InputOTP maxLength={6} value={otp} onChange={setOtp} disabled={isVerifying || isResending} autoFocus>
            <InputOTPGroup>
        ...
        <Button onClick={handleVerify} className="w-full" size="lg" disabled={isVerifying || isResending || otp.length !== 6}>
